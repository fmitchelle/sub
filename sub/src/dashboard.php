  ...
  <div class="col-lg-9">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        ...
        <div class="d-flex gap-2">
            <!-- Loading Indicator -->
             <div id="updateStatus" class="d-none align-self-center small muted">در حال بروزرسانی...</div>
             <button id="btnUpdate" class="btn btn-accent" onclick="showUpdateModal()">
                <span id="btnIcon">🔄</span> بروزرسانی و تست
             </button>
        </div>
    </div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content" style="background:var(--card); border:1px solid var(--border); color:var(--text);">
      <div class="modal-header border-bottom-0">
        <h5 class="modal-title">بروزرسانی سرورها</h5>
      </div>
      <div class="modal-body">
        <div class="mb-3 border-bottom border-secondary pb-3">
             <div class="form-check form-switch mb-2">
                <input class="form-check-input" type="checkbox" id="chkRemoveInactive">
                <label class="form-check-label" for="chkRemoveInactive">حذف سرورهای غیرفعال (Timeout)</label>
            </div>
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="chkRemovePersian" checked>
                <label class="form-check-label" for="chkRemovePersian">حذف کانفیگ‌های تبلیغاتی (متن فارسی)</label>
            </div>
        </div>
        <div class="d-grid mb-3">
             <button class="btn btn-accent" id="btnStartUpdate" onclick="triggerUpdate()">شروع عملیات</button>
        </div>
...
async function triggerUpdate() {
    ...
    document.getElementById('btnStartUpdate').innerText = 'در حال پردازش...';
    ...
    // Get Options
    const removeInactive = document.getElementById('chkRemoveInactive').checked;
    const removePersian = document.getElementById('chkRemovePersian').checked;
    const queryParams = `&remove_inactive=${removeInactive}&remove_persian=${removePersian}`;
    ...
    try {
        ...
        for (const src of sources) {
            ...
            try {
                const res = await fetch(`api.php?action=process_source&id=${src.id}${queryParams}`);
        ...
        try {
            const mRes = await fetch(`api.php?action=process_manual${queryParams}`);
